...
def contains_char(word: str, letter: str) -> None:
    ...
    )  # string concatenation; matches the instructions
    count: int = 0  # keeps count of the number of matching characters
    if (
        letter == word[0]
    ):  # if the character is the same as the index of the word, like letter("h") = word[0]("hello" [0]): "h"
        count = count + 1  # increase the count variable above by 1
    ...
    if (
        count == 0
    ):  # If there are no instances of the input letter found in the input word then move into the body.
        ...
        )  # mulitple matches found between the input character and input word
...
# pulls the functions together without having to call each function repeatedly
